...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        self.x_0_pred_coef_2 = -1 * torch.sqrt(torch.divide(1 - self.alphas_cumprod, self.alphas_cumprod)) 
    ...
    def model_predictions(self, x_t, t):
        ...
        # x_0 = ((x_0 - torch.min(x_0))/(torch.max(x_0) - torch.min(x_0)) - 0.5)*2
    ...
    def predict_denoised_at_prev_timestep(self, x, t: int):
        ...
        # print(t)
        if t.all()>0:
            z = torch.randn(posterior_mean.shape)
        else: 
            z = 0
        pred_img = posterior_mean + torch.sqrt(posterior_variance) * z.cuda()
        return pred_img, x_0
    ...
    def sample_times(self, total_timesteps, sampling_timesteps):
        ...
        # times = [i for i in range(total_timesteps-1, -1,-int(total_timesteps/sampling_timesteps))]
    ...
    def get_time_pairs(self, times):
        ...
        # time_pairs = [(tau_i, tau_i-1) for tau_i in times]
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ...
        coeff_eps_i = torch.sqrt(1 - alpha_isub1 - sigma_i)
        # sample from q(x_{\tau_{i - 1}} | x_{\tau_t}, x_0)
        # HINT: use the reparameterization trick
        mean_i = torch.sqrt(alpha_isub1) * x_0 + coeff_eps_i  * pred_noise
    ...
    def sample_given_z(self, z, shape):
        ...
        sample_fn = self.sample_ddpm if not self.is_ddim_sampling else self.sample_ddim
        # z = view(shape, device = self.betas.device)
        return sample_fn(shape, z.reshape(z.shape[0], 32,32,3).permute(0,3,1,2))
